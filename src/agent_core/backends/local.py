...
from agent_core.backends.result import (
...
class LocalBackend(TerminalBackend):
    ...
    def __init__(
        ...
        *,
        ...
        snapshot = snapshot_path or str(self._cwd / ".agent_core_session.json")
        self.snapshot = SessionSnapshot(Path(snapshot), excluded_vars=excluded_vars)
        self.dump_dir = Path(dump_dir) if dump_dir else (self._cwd / ".agent_core_dumps")
