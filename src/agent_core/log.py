...
Предоставляет единый корневой логгер ``agent_core`` и функцию конфигурации,
которая:

- задаёт уровень из конфигурации или переменной окружения ``AGENT_CORE_LOG_LEVEL``;
...
def get_logger(name: str | None = None) -> logging.Logger:
    """Возвращает логгер под именем ``agent_core[.<name>]``."""
    if name:
...
def configure(
    ...
    Параметры:
        level: уровень логирования (по умолчанию — из ``AGENT_CORE_LOG_LEVEL``).
        to_file: добавлять ли файловый обработчик.
        profile: профиль, в каталог логов которого пишется файл.
        filename: имя файла лога (по умолчанию ``agent_core.log``).
    """
    root = get_logger()
    ...
    if to_file:
        ...
        if existing is None:
            ...
            fh = logging.FileHandler(
                logs_dir / (filename or "agent_core.log"), encoding="utf-8"
            )
