...
class ProviderRegistry:
    ...
    def discover(self, user_modules: Iterable[Callable[[ProviderRegistry], None]] = ()) -> None:
        ...
        if self._discovered:
            return
        from prokop.providers import builtin
...
def _entry_points() -> Iterable:
    """Точки входа ``prokop.providers`` (если среда их предоставляет)."""
    try:
        from importlib.metadata import entry_points

        eps = entry_points()
        if hasattr(eps, "select"):
            return list(eps.select(group="prokop.providers"))
        return list(eps.get("prokop.providers", ()))  # type: ignore[union-attr]
