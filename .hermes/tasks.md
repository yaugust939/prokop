...
security. Сессия 2 (8d978e0) — файловая память, адаптер Telegram.
Сессия 3 — **адаптер MAX** (приоритет владельца).
...
- [x] P1 · hermes · **Изменение max-adapter: полный адаптер российского мессенджера MAX** (#25)

## Осталось

- [ ] P1 · hermes · Живая проверка адаптера MAX с настоящим токеном бота (нужен токен с business.max.ru) (#26)
- [ ] P2 · hermes · Вебхуки MAX (`POST /subscriptions`) — нужен публичный HTTPS и сертификат Минцифры (#27)
- [ ] P2 · hermes · Изоляция процессов/ФС (настоящая песочница) — платформенно-специфична (#18)
- [ ] P2 · hermes · Остальные платформы обмена сообщениями (#19)
- [ ] P2 · hermes · Синхронизировать и закрыть изменение orchestrator (0/29 задач) (#20)
- [ ] P2 · hermes · Проверить `docker build` в среде с доступным реестром образов (#22)

## Заметки

- **MAX**: официальная документация `dev.max.ru/docs-api`, OpenAPI-схема
  `github.com/max-messenger/api-schema` (скачана и разобрана). Ключевое:
  база `platform-api2.max.ru`, заголовок `Authorization`, `marker` вместо
  смещения, `mid` — строка, действия только в группах, медиа в два шага,
  адресация диалога через `user_id`.
- Коммиты: f2e5adc (portability + CLI/TUI/MCP/checkpoints/security),
  8d978e0 (memory + Telegram), сессия 3 — MAX.
- 537 тестов; `openspec validate --all`: 17 passed; 16 спек, 11 в архиве.
